...
def _stock_panel(rows: int = 420) -> pd.DataFrame:
    ...
    for idx, symbol in enumerate(symbols):
        frame = make_synthetic_ohlcv(rows=rows, seed=10 + idx).copy()
        scale = 1.0 + idx * 0.12
...
def test_blackcat_research_cli_smoke(tmp_path, monkeypatch) -> None:
    ...
    etf = make_synthetic_ohlcv(rows=420, seed=3)
    ...
    config.search.top_candidates_per_family = 2
    config.ranking.filters.min_closed_trades = 1
    ...
    assert Path("runs/blackcat-smoke/leaderboard.csv").exists()
    assert Path("runs/blackcat-smoke/summary.json").exists()
    assert Path("runs/blackcat-smoke/stock_family_summary.csv").exists()
