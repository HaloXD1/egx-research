...
def _stock_panel(rows: int = 420) -> pd.DataFrame:
    ...
    for idx, symbol in enumerate(symbols):
        frame = make_synthetic_ohlcv(rows=rows, seed=60 + idx).copy()
        scale = 1.0 + idx * 0.06
...
def test_soft_filter_research_cli_smoke(tmp_path, monkeypatch) -> None:
    ...
    etf = make_synthetic_ohlcv(rows=420, seed=23)
    ...
    assert Path("runs/soft-smoke/leaderboard.csv").exists()
    assert Path("runs/soft-smoke/summary.json").exists()
    assert Path("runs/soft-smoke/stock_variant_summary.csv").exists()
