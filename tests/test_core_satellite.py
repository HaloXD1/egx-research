...
def test_stock_core_satellite_cli_smoke(tmp_path, monkeypatch) -> None:
    ...
    result = CliRunner().invoke(
        app,
        [
            "stock-core-satellite-backtest",
            "--config",
            "config/stock_rotation.yaml",
            "--run-id",
            "core-satellite-smoke",
            "--top-n",
            "2",
            "--core-weight",
            "0.7",
    ...
    run_dir = Path("runs/core-satellite-smoke")
    for filename in [
        "selected_holdings.csv",
        "equity_curve.csv",
        "trade_actions.csv",
        "turnover.csv",
        "trade_actions_adaptive_balanced.csv",
        "turnover_adaptive_balanced.csv",
        "trade_actions_adaptive_aggressive.csv",
        "turnover_adaptive_aggressive.csv",
        "summary.json",
    ]:
        assert (run_dir / filename).exists()

    selected = pd.read_csv(run_dir / "selected_holdings.csv")
    first = selected[selected["rebalance_date"] == selected["rebalance_date"].min()]
    ...
    assert summary["core_satellite_metrics"]["final_equity"] > 0
    assert summary["adaptive_balanced_metrics"]["final_equity"] > 0
    assert summary["adaptive_aggressive_metrics"]["final_equity"] > 0
